...
static uint8_t m_custom_value[CAN_MAIN_UUID_LEN] = {0};
...
static ble_uuid_t m_adv_uuids[] = {{RCDIY_SERVICE_UUID, BLE_UUID_TYPE_BLE}}; 
...
static void notification_timeout_handler(void * p_context)
{
    ...
    m_custom_value[0] = pid & 0xFF;
    m_custom_value[1] = (pid >> 8) & 0xFF;
    m_custom_value[2] = (pid >> 16) & 0xFF;
    m_custom_value[3] = (pid >> 24) & 0xFF;
    m_custom_value[4] = count++;
    
    NRF_LOG_INFO("Send data");
    err_code = ble_candata_update(&rcdiy_service, m_custom_value);
...
static void on_cus_evt(ble_cus_t  *p_cus_service, ble_cus_evt_t * p_evt)
{
    ...
    switch(p_evt->evt_type)
    {
        ...
        case BLE_CUS_EVT_CONNECTED:
            NRF_LOG_INFO("Connected EVT");
            break;

        case BLE_CUS_EVT_DISCONNECTED:
            NRF_LOG_INFO("Disconnected EVT");
              break;

        default:
            NRF_LOG_INFO("EVT Event %d",p_evt->evt_type);
...
static void services_init(void)
{
    ...
    err_code = ble_cus_init(&rcdiy_service, &cus_init);
    APP_ERROR_CHECK(err_code);	
}
